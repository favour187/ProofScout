const CACHE='proofscout-v18';
const ASSETS=['./','./index.html','./privacy.html','./proofscout-offline.html','./demo-target.html','./styles.css','./app.js','./scout.js','./scout.css','./manifest.webmanifest','./icon.svg','./favicon.ico','./apple-touch-icon.png'];
self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(ASSETS)).then(()=>self.skipWaiting())));
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
self.addEventListener('fetch',e=>{
  if(e.request.method!=='GET')return;
  const url=new URL(e.request.url);
  const isDoc=e.request.mode==='navigate'||url.pathname.endsWith('.html')||url.pathname.endsWith('/')||url.pathname.endsWith('/ProofScout');
  if(isDoc){
